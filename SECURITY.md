...
| **公网滥用（烧掉宿主的模型额度）** | Web 端按 IP 限流（`STYLOTRACE_WEB_RATE`，本机不限）；隧道/反代下取真实来源 IP，且只在直连方是内网时才采信转发头，防伪造绕过 |
| **请求体撑爆内存** | `/api/*` 请求体上限 8MB（`STYLOTRACE_WEB_MAX_BODY`）：先查 `Content-Length` 返回 413，缺失时由流式读取兜底断开 |
| **结构性 AI 腔被误当硬失败** | AI 腔模式目录（§1–§25）只作**建议**，不参与 `passed` 交付闸门，避免本该交付的稿子被反复重写 |
...
- [ ] 退让协议测试通过（外部修改不覆盖）
- [ ] 全量测试绿（引擎 + 插件 + 浏览器验收）
- [ ] 公网部署设了 `STYLOTRACE_WEB_PASSWORD`（不带密码 = 任何人可用你的模型额度）
- [ ] 公网部署实测：无凭证 `/api/*` 返回 401；超 8MB 请求体返回 413；伪装公网来源连打触发 429
